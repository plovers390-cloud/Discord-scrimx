    ...
    async def reg_open_msg(self):
        ...
        if len(self.open_message) <= 1:
            return discord.Embed(
                ...
                description=f"<a:megaphone:1431683757697536000> **`{self.required_mentions}`** mentions required.\n"
                f"<a:megaphone:1431683757697536000> Total slots: **`{self.total_slots}`** [`{reserved_count}` slots reserved]",
            )
