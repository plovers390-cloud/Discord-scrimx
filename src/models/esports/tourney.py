...
class Tourney(BaseDbModel):
    ...
    name = fields.CharField(max_length=30, default="ScrimX-Tourney")
    ...
    async def __start_registrations(self):
        ...
        _e.description = (
            ...
            "```"
            f"<a:megaphone:1431683757697536000> {self.required_mentions} mentions required.\n"
            f"<a:megaphone:1431683757697536000> Total slots: {self.total_slots} [{self.total_slots - await self.assigned_slots.all().count()} slots left]"
            "```"
        )
