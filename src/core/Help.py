...
class HelpCommand(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping: Mapping[Cog, List[commands.Command]]):
        ...
        server_link = f"[Privacy Policy](https://discord.gg/QyhbVffzke)"

        embed.description = f"{server} **|** {invite} **|** {server_link}\n\n"

        guild = await Guild.get_or_none(pk=ctx.guild.id)
        if guild and guild.is_premium:
            embed.description += f"<:Vip:1431647687261949994> [__Server Premium ending:__]({config.SERVER_LINK}) {discord_timestamp(guild.premium_end_time)}"
